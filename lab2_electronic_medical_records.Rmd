...
glimpse(raw.data)
```
##note_id is a unique number for each medical note in the dataset — there are 4,999 in total.
##description gives a quick summary of the patient's case, like why they came in or what symptoms they had.
##medical_specialty tells you which medical field the note belongs to, like Cardiology or Allergy/Immunology.
##sample_name is a short title or label for the note, often naming the condition or type of visit.
##transcription is the full medical note written by the doctor which includes all the details of the patient such as their age,gender, and presenting complaints.
##keywords are important medical terms pulled from the note to help quickly understand what the note is about.
...
```
## There are 40 medical specialties featured in these notes.
...
```
##Surgery has 11,977 unique unigrams.
##Orthopedic has 7,682 unique unigrams across 98,129 total word instances.
##Radiology has 5,935 unique unigrams across 42857 total word instances.
...
```
##Surgery has 130,404 unique bi-grams — the highest among the three.
##Orthopedic has 55,732 unique bi-grams.
##Radiology has 28,297 unique bi-grams.
...
```
##Surgery has 29,652 unique sentences — the most among all categories.
##Orthopedic has 11,174 unique sentences.
##Radiology has 4,565 unique sentences.
...
##Probably not. General-purpose lemmatizers are built for everyday English, so they don’t really understand how medical professionals write.In clinical notes, you’ll often find phrases like “prepped and draped”, “postoperative diagnosis”, or “carotid artery” — and a general lemmatizer might either break them apart incorrectly or not recognize them as meaningful medical terms. It might also fail to group related words like “procedures” and “procedure”, or leave out abbreviations and clinical shorthand entirely. Medical text has a ton of domain-specific language, and without a medical-aware lemmatizer, we risk losing important context. So while a general one might help a little, it’s not really reliable for health data. Tools built specifically for medical text would do a much better job
...
##Radiology vs Orthopedic: The dots are mostly scattered below the diagonal line. This means the lemmas tend to appear more frequently in Radiology than in Orthopedic. For example, terms like “series images”, “canal neural”, and “acute fracture” are more common in radiology reports.This makes sense. Radiology focuses on imaging and interpreting results, while Orthopedic is more procedure-focused. Their vocabularies overlap less.
## Surgery vs Orthopedic:Here, the dots are much closer to the diagonal. That means lemmas are used at similar frequencies in both Surgery and Orthopedic notes. Words like “carpal ligament”, “femoral component”, and “antebrachial fascia” are frequent in both.That’s expected too. Orthopedic is a surgical specialty, so it naturally shares much of the same terminology and documentation style with general Surgery.
##The plot confirms what we would expect. Surgery and Orthopedic speak a very similar language, while Radiology has a more distinct vocabulary tailored to imaging and diagnostics.
...
# Step 1: Compute proportions from lemmatized data
lemma.freq <- lemmatized.data %>% 
...
# Step 2: Filter for Surgery and Radiology only
surgery_radiology_data <- lemma.freq %>%
  filter(medical_specialty %in% c("Surgery", "Radiology")) %>%
  select(lemma, proportion, medical_specialty)

# Step 3: Reshape into wide format
comparison_data <- surgery_radiology_data %>%
  pivot_wider(names_from = medical_specialty, values_from = proportion) %>%
  drop_na()

# Step 4: Plot
ggplot(comparison_data, aes(x = Radiology, y = Surgery)) +
  geom_point(alpha = 0.4) +
  geom_abline(linetype = "dashed") +
  geom_text(aes(label = lemma), check_overlap = TRUE, vjust = 1.5, size = 3) +
  scale_x_log10(labels = scales::percent_format(accuracy = 0.001)) +
  scale_y_log10(labels = scales::percent_format(accuracy = 0.001)) +
  labs(
    x = "Radiology Lemma Frequency",
    y = "Surgery Lemma Frequency",
    title = "Comparison of Lemma Frequencies: Surgery vs Radiology"
  ) +
  theme_minimal()
```
...
##Looking at the tf_idf values, a high tf_idf value means that the lemma stands out as uniquely frequent in one specialty and rare elsewhere.
##Range motion has the highest tf-idf in the Orthopedic list, meaning it's very frequent in Orthopedic notes and infrequent in others, making it distinctive.
##"Myocardial perfusion” has a high tf-idf, meaning it’s a signature term in Radiology — likely rare in other specialties.
##“Anterior chamber” has the highest tf-idf here — frequent in Surgery, rare elsewhere.

##In Orthopedics, terms like “range motion”, “carpal ligament”, and “steri strips” are common — all tied to joints, bones, and post-op care.In Radiology, words like “myocardial perfusion”, “calcific plaque”, and “needle emg” reflect imaging and diagnostics.In Surgery, phrases like “lithotomy position”, “suture ligated”, and “endotracheal anesthesia” point to procedures and operating room steps.These lemmas stand out because TF-IDF highlights what’s unique to each specialty — filtering out general terms and surfacing what really defines their language.
...
specialty_gamma_6 <- dplyr::left_join(specialty_gamma_6, note_id_specialty_mapping)
```
```{r}
specialty_gamma_6 %>%
  dplyr::mutate(medical_specialty = reorder(medical_specialty, gamma * topic)) %>%
  ...
  ggplot2::labs(x = "Topic", y = expression(gamma))
```
##Yes, most of the top terms from the 5-topic model still show up in the 6-topic version. For example, phrases like "prepped draped", "procedure patient", "blood loss", "tolerated procedure", and "preoperative diagnosis" appear across multiple topics in both models. These are common in surgical notes, so it makes sense that they continue to stand out.
##Looking at the boxplot for the 6-topic model:Orthopedic still leans heavily toward Topic 1, but there’s a bit more spread into Topics 2 and 4, hinting at some variation within the specialty.Surgery is now more clearly distributed across several topics (especially Topics 1 through 5), which makes sense given how diverse surgical language can be.Radiology remains fairly focused but now splits between Topics 3 and 6, possibly separating notes about cardiac imaging (like “coronary artery”) from more general radiology content.
