...
##Yes, the filenames actually give away whether the image shows a normal case or pneumonia, and even mention if it’s bacterial or viral. This can make it harder to build a good model because instead of learning from the chest X-rays, the model might just pick up clues from the filenames. As a result, it could perform well during training but fail when tested on real-world images that don’t follow the same naming pattern.
...
##Yes, even from just a few images, we can see differences in brightness, contrast, and how the chest is positioned. Some images are lighter or darker, and the lungs aren’t always centered the same way. These differences can confuse a model, so it’s important to normalize things like lighting, size, and alignment before training.
...
##If the dataset wasn’t balanced, we could use a few helpful tricks to fix that. One way is to oversample the smaller class—basically making more copies or slightly altered versions of those images. Another way is to undersample the bigger class by using fewer of those images. We could also tell the model to pay more attention to the minority class by giving it a higher weight during training. Lastly, we could use data augmentation to create new, slightly different images from the existing ones to even things out. These methods help the model learn from both classes more fairly.
...
##No, the model isn't really learning to tell apart pneumonia from normal X-rays. The validation accuracy stays stuck at 50% in every epoch, which means it's just guessing. Even though training accuracy slightly improves, it’s not enough — and the gap shows that the model is not generalizing well.
##From the plots, we see that the test loss barely changes, and test accuracy stays flat, while training values bounce around a bit. This suggests the model either doesn't have enough capacity, isn’t trained long enough, or the input data isn’t set up properly (like preprocessing or feature scaling). It's also possible the model is overfitting or just not picking up useful features from the images.
##To fix this, we’d need to tune the model, improve preprocessing, and maybe train for more epochs.
...
##To improve this model, I’d suggest a few things. First, try training it for more epochs — three is probably not enough for the model to learn meaningful patterns, especially from images. Next, making sure the images are properly preprocessed — such as resizing, normalizing pixel values, and possibly data augmentation can really help. We could also try using a more powerful model architecture, maybe something like a pretrained CNN (e.g., ResNet), since those are better at handling image data. Lastly, double-checking the labels and balance in the dataset to make sure everything is clean and consistent. These changes should give the model a better shot at actually learning to classify the X-rays.
...
##While data augmentation can be really helpful, it’s not perfect and comes with some downsides. One issue is that if we overdo the transformations, we might end up with images that look unnatural or unrealistic — and that can actually confuse the model instead of helping it learn. Also, augmentation doesn’t add any truly new information, just variations of what we already have, so it won’t solve problems like poor data quality or missing labels. Plus, it can slow down training because the model has to process more data and transformations on the fly. So, while it’s a great tool, it needs to be used carefully and alongside other improvements.
...
##ImageNet, specifically the ImageNet Large Scale Visual Recognition Challenge (ILSVRC) 2012, is a massive image classification competition that really pushed forward the field of deep learning. It involves over 1.2 million training images across 1,000 different object categories—everything from animals and vehicles to everyday objects. This helps us because models trained on ImageNet have already learned to recognize a wide range of visual features like edges, textures, shapes, and patterns. By using these pre-trained models through transfer learning, we can skip the heavy lifting and fine-tune them for our specific task (like detecting pneumonia from X-rays) with much less data and time. It's like giving your model a head start with built-in visual knowledge.
...
##MobileNet is a lightweight and efficient convolutional neural network designed for speed and low computational cost, especially on mobile and embedded devices. What makes it powerful is that it uses something called depthwise separable convolutions, which dramatically reduces the number of parameters without losing too much accuracy.
##Using MobileNet in this practical assignment can really help because it’s already trained on a huge dataset like ImageNet and has learned general image features. So instead of training a model from scratch, we can fine-tune MobileNet on our chest X-ray dataset. It’s fast, doesn’t require a lot of computing power, and still performs well — making it ideal for small datasets or environments where resources are limited. Plus, because it’s lightweight, it’s less likely to overfit and gives us a better shot at improving accuracy right away.

**9_** How many parameters does this network have? How does this compare to better performing networks available in torch?

##MobileNet has around 4.2 million parameters, which is quite small compared to other popular deep learning models. For example, ResNet-50 has about 25 million parameters, and VGG16 has over 130 million. This smaller size makes MobileNet much faster and more efficient, especially for training on smaller datasets or running on limited hardware. While it might not always match the top performance of larger networks like ResNet or EfficientNet, it still gives solid results — especially when speed and efficiency matter. So, it's a great choice when you want a good balance between performance and resource use.
...
# Freeze earlier layers if you want to use it as a feature extractor
# for (param in resnet$parameters) {
#   param$requires_grad_(FALSE)
# }

# Replace the final layer to match your classification task (2 classes)
resnet$fc <- nn_linear(in_features = 512, out_features = 2)
...
optimizer <- optim_adam(resnet$parameters, lr = 0.001)
...
train_transforms <- function(img) {
  ...
  # If grayscale image [H, W], add channel dim to become [1, H, W]
  if (img$ndim == 2) {
    img <- img$unsqueeze(1)
  }

  # If 1-channel image, repeat it to become 3 channels using torch_cat
  if (img$shape[1] == 1) {
    img <- torch_cat(list(img, img, img), dim = 1)
  }

  return(img$to(dtype = torch_float()))  # Ensure it's float
}
```
```{r}
train_dataset <- ImageDataset_augment(train_X, train_y, transform = train_transforms)
val_dataset <- ImageDataset_augment(val_X, val_y, transform = train_transforms)
```
```{r}
train_dataloader <- dataloader(train_dataset, batch_size = 16)
val_dataloader <- dataloader(val_dataset, batch_size = 16)
```
```{r}
dim(train_dataloader$.iter()$.next()[[1]])
# Should print: [1] 16  3 224 224
```
...
# 2. Load MobileNet and replace final layer
mobilenet <- model_mobilenet_v2(pretrained = TRUE)
mobilenet$classifier <- nn_linear(1280, 2)

# 3. Define a luz model class wrapping MobileNet
# 3. Define a custom model class using torch::nn_module
Net <- nn_module(
  initialize = function() {
...
fitted <- Net %>%
  ...
  fit(train_dataloader, epochs = 3, valid_data = val_dataloader)
```
##The second model does not perform better. Although it shows a slightly higher training accuracy (0.521 vs. 0.516 in the first model), its validation loss is significantly worse (0.8057 compared to ~0.693 in the first), and the validation accuracy remains stuck at 0.5 in both models. This suggests that the second model may be overfitting slightly to the training data without any improvement in generalization. Therefore, the first model performs better overall in terms of both validation loss and training stability.
```{r}
library(gridExtra)
```
```{r}
if (!requireNamespace("gridExtra", quietly = TRUE)) {
  stop("Package 'gridExtra' is required but not installed. Please install it before knitting.")
}
library(gridExtra)
```
